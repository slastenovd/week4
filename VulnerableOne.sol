...
Кто угодно может вызвать функции set_super_user и add_new_user, и соответсвенно сможет сделать кого угодно суперюзером
Как вариант, следует переписать конструктор таким образом, чтобы он делал первого пользователя суперюзером без вызовов функций
set_super_user() и add_new_user()

		is_super_user[msg.sender] = true;
		users_map[msg.sender] = UserInfo({ created: now, ether_balance: 0 });
...
/*
Logic bug
Отсутствует проверка прав на запуск функции. В соответствии с комментом выше: Every superuser can add new users and superusers
Должно быть:
...
/*
Следует ограничить количество итераций для избежания out_of_gas condition
